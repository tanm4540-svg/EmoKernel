...
class EmotionAnalyzer:
    ...
    def analyze(self, text: str) -> Dict[str, float]:
        ...
        # 最优：sentence-transformers embedding（如果可用）
        if has_model() and self._node_embeddings:
            return self._embedding_analyze(text)

        # 次优：关键词 + n-gram 投票集成
        kw_result = self._keyword_analyze(text)
        if self._ngram_clf is not None:
            ng_result = self._ngram_clf.predict(text, top_k=5)
    ...
    def _keyword_analyze(self, text: str) -> Dict[str, float]:
        ...
        for label, keywords in self._keyword_map.items():
            ...
            if count > 0:
                scores[label] = min(count / len(keywords) * 3, 1.0)
